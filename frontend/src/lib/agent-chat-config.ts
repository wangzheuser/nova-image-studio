...
export const AGENT_TEXT_MODEL_FALLBACK = 'gpt-4o-mini';
...
export interface AgentProposal {
  ...
  /** GPT Image 2 fast/plus 质量参数 */
  gptImageQuality?: GptImageQuality;
  /** GPT Image 2 fast/plus 风格参数；自动时不传给上游 */
  gptImageStyle?: GptImageStyle;
  /** GPT Image 2 fast/plus 背景参数 */
...
- gpt_image_quality：当用户明确要求 GPT Image 2 fast/plus 的质量档位时填 "high"/"medium"/"low"，无明确需求给 "auto" 或 null。
  ...
  parameters: {
    ...
    properties: {
      ...
      gpt_image_quality: {
        ...
        description: 'GPT Image 2 fast/plus 质量参数；无明确需求给 auto 或 null',
      },
      gpt_image_style: {
        type: ['string', 'null'],
        enum: ['vivid', 'natural', null],
        description: 'GPT Image 2 fast/plus 风格参数；自动时给 null',
      },
      gpt_image_background: {
        type: ['string', 'null'],
        enum: ['auto', 'transparent', 'opaque', null],
        description: 'GPT Image 2 fast/plus 背景参数；无明确需求给 auto 或 null',
      },
    },
    required: [
      'action',
